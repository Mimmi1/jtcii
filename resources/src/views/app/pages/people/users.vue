      ...
      <b-modal hide-footer size="lg" id="New_User" :title="editmode?$t('Edit'):$t('Add')">
        <b-form @submit.prevent="Submit_User" enctype="multipart/form-data">
          <b-row>
            ...
            <!-- assigned_warehouses -->
            <b-col md="4" sm="4">
              <h5>{{$t('Assigned_Stores')}}</h5>
            </b-col>

            <b-col md="8" sm="8">
              <label class="checkbox checkbox-primary mb-3"><input type="checkbox" v-model="user.is_all_warehouses"><h5>{{$t('All_Stores')}} <i v-b-tooltip.hover.bottom title="If 'All Warehouses' Selected , User Can access all data for the selected Warehouses" class="text-info font-weight-bold i-Speach-BubbleAsking"></i></h5><span class="checkmark"></span></label>
               
               <b-form-group class="mt-2" :label="$t('Some_Stores')">
                  <v-select
                    multiple
